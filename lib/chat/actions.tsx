...
import { Contents } from '@/components/tools/contents'
...
async function submitUserMessage(content: string) {
  ...
  ;(async () => {
    try {
      const result = await streamText({
        ...
        tools: {
          ...
          suggestDifferentQuestions: {
            description: 'Suggest a different question related to the tags.',
            parameters: z.object({
              questions: z.array(
                z
                  .string()
                  .describe(`List of questions that are related to the tags.`)
              )
            })
          }
        },
      ...
      Example questions:
      - "What are some success stories of Korean founders?"
      - "How do I overcome the language barrier when pitching to investors?"
      `,
        messages: [...history]
      })
      ...
      for await (const delta of result.fullStream) {
        ...
        if (type === 'text-delta') {
          ...
          if (toolName === 'showRelatedContents') {
            ...
            // TODO: Implement the suggestDifferentQuestions tool
            uiStream.update(
              <BotCard>
                <Contents tags={questions} />
              </BotCard>
            )
      ...
      display:
        message.role === 'assistant' ? (
          message.display?.name === 'showRelatedContents' ? (
            <BotCard>
              <Contents tags={message.display.props.tags} />
            </BotCard>
          ) : message.display?.name === 'suggestDifferentQuestions' ? (
            // TODO: Implement the suggestDifferentQuestions tool
            <BotCard>
              <Contents tags={message.display.props.questions} />
            </BotCard>
          ) : (
